...
def main():
  ...
  while(1):
    ...
    #Get eNB
    #eNB = int(lte_measurement["net_param"]["eNB"])
    eNB = 12345678
    BTS = CellFind_Connected(uav_coo, eNB, BTS_Distance)
    print(eNB)

    #If disconnected or eNB does not exist
    if (BTS == 0):
        print("No eNB")
        BTS = BTS_Distance.iloc[0]
        print("Connected to BTS not found, nearest is " + BTS.site_name + " with ID " + str(BTS.node_id) + " at distance " + str(round(BTS.distance,2)) + " m, with Bearing " + str(round(BTS.bearing, 0)) + "°, UAV Head is " + str(uav_head) + "°")
    else:
        print("Connected to the BTS " + BTS.site_name + " with ID " + str(BTS.node_id) + " at distance " + str(round(BTS.distance,2)) + " m, with Bearing " + str(round(BTS.bearing, 0)) + "°, UAV Head is " + str(uav_head) + "°")
        BTS = BTS.iloc[0]
    #Print BTS data
    print(BTS)
    ...
    with open('LTE_Ant_Sierra_conn.json', 'a') as outfile:
